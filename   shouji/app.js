class SanghuangDetector {
    constructor() {
        ...
        this.preferredSliceSize = this.isMobile ? 480 : 640;
        this.preferredOverlap = this.isMobile ? 0.3 : 0.25;
        
        console.log(`📱 设备检测: ${this.isMobile ? '移动端' : '桌面端'}`);
        console.log(`⚡ 性能模式: ${this.isLowPerformance ? '低性能' : '高性能'}`);
        console.log(`📐 推荐切片大小: ${this.preferredSliceSize}, 重叠率: ${this.preferredOverlap}`);
    ...
    detectMobile() {
        ...
        const tabletRegex = /iPad|Android(?!.*Mobile)|IEMobile.*Tablet|Silk.*Accelerated/i;
        
        const isMobileAgent = mobileRegex.test(navigator.userAgent);
        const isTabletAgent = tabletRegex.test(navigator.userAgent);
        const hasTouchPoints = navigator.maxTouchPoints && navigator.maxTouchPoints > 0;
        const isSmallScreen = window.innerWidth <= 768;
        
        return isMobileAgent || isTabletAgent || (hasTouchPoints && isSmallScreen);
    }
    ...
    initUI() {
        ...
        if (this.isMobile) {
            container.classList.add('mobile-ui');
            container.classList.remove('desktop-ui');
            console.log('🔄 切换到移动端UI');
        } else {
            container.classList.add('desktop-ui');
            container.classList.remove('mobile-ui');
            console.log('🔄 切换到桌面端UI');
        }
    }
    ...
    async detect() {
        ...
        try {
            // 🔪 统一走切片推理（小图会自动跳过切片）
            // 移动端使用更小的切片和更大的重叠提升检测质量
            const sliceSize = this.isMobile ? 480 : 640;
            const overlap = this.isMobile ? 0.3 : 0.25;
            const detections = await this.detectWithSlicing(this.currentImage, sliceSize, overlap);
    ...
    async detectWithSlicing(image, sliceSize = 640, overlap = 0.25) {
        ...
        for (let i = 0; i < uniq.length; i++) {
            ...
            dets.forEach(d => {
                allDetections.push({
                    ...d,
                    x1: Math.max(0, d.x1 + s.x),
                    y1: Math.max(0, d.y1 + s.y),
                    x2: Math.min(image.width, d.x2 + s.x),
                    y2: Math.min(image.height, d.y2 + s.y),
                });
            });
        }
    ...
    postprocessResults(results) {
        ...
        for (let i = 0; i < numDetections; i++) {
            ...
            const x1 = Math.max(0, (x1_640 - this.padX) / this.scale);
            const y1 = Math.max(0, (y1_640 - this.padY) / this.scale);
            const x2 = Math.min(this.currentImage.width, (x2_640 - this.padX) / this.scale);
            const y2 = Math.min(this.currentImage.height, (y2_640 - this.padY) / this.scale);
